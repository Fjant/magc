...
var postPath = path + "/msg/mPMsg!list.action";
$(function() {
	// 调用初始化数据
	ajaxresult();
	// 加载初始化数据
	function ajaxresult() {
		$('#mPMsglist').html("<p style='padding:5px;font-size:13px;'>正在加载中...</p>");
				...
				if(result != null && result.length > 0) {
					for (var i = 0; i < result.length; i++) {
			    		 var temp = '<tr class="listTr">'		    
				    			 ...
				    			 +'<td id="openId'+result[i].id+'" style="width:10%;">' + (result[i].openId == null ? "" : result[i].openId) + '</td>' 
							     +'<td style="width:8%;">' + (result[i].customerName == null ? "" : result[i].customerName) + '</td>' 
...
function deleteMsg(id){
	try {
		...
		if(returnValue){
			var openId = $("#openId"+id).text();
			var statusId = $("#statusId"+id).text();
			if(statusId== "已发送"){
				alert("您要删除的消息【微信号："+openId+"】已经发送，不能够删除!");
			}else{
				var openIds1 = '';
				var openIds2 = '';
				var a = 0;
				...
				if((id=="undefined"||id==null||id<=0)&&b==0){
					alert("您要删除的消息【微信号："+openIds1+"】已经发送，不能够删除!");
				}else{		
					...
					$.post(postPath, param,
						...
						if(date.retCode == 1){
							if(a > 0){
								alert("您要删除的消息【微信号："+openIds2+"】删除成功,【微信号："+openIds1+"】已经使用,不能够删除!");
							}else{					
								alert("删除成功");
							}
...
function exportRrport() {
	...
	try {
		$.doPostAsync(path + "/msg/mPMsg!exportMPMsg.action", paramArray, 
		  function(data) {
