...
@Service
public class KaRefundJob implements BaseJob {

	private static Logger logger = LoggerFactory.getLogger(KaRefundJob.class);
	private static String KA_REFUND_URI = SystemConfig.getPara("KadanServiceUri") + "/kaRefund";
	...
	public void execute(JobEntity jobEntity) {
		try {
			...
			try {
				logger.info("初始化退款写账数据源及脚本");

				sqlBundle = ResourceBundle.getBundle("jdbc_sqls");
				selectRefund = sqlBundle.getString("KADAN_REFUND_KA_SELECT");

				kadanDBJdbc = new JdbcHelper(DataSourceUtil.getDataSource("oracle_kadan"));
			} catch (Exception ex) {
				logger.error("初始化连接失败，执行退款写账任务终止 。", ex);
				isHasError = true;
			}
			...
			if (!isHasError) {
				try {
					...
					logger.info("查询待退款写账数据--开始");
					// 1、查询本地数据库获取数据
					List<String> bizList = kadanDBJdbc.queryForStringList(selectRefund);
					logger.info("查询待退款写账数据--结束");

					bizCount = bizList.size();

					if (bizList.size() > 0) {
						Map<String, String> paramsMap = new HashMap<String, String>();
						for (String refundId : bizList) {
							try {
								long timestamp = new Date().getTime();
								paramsMap.put("refundId", refundId);
								...
								String params = "refundId=" + refundId + "&sign=" + sign + "&timestamp=" + timestamp;
								String result = HttpHelper.httpRequest(KA_REFUND_URI, "POST", params);
								logger.info("退款写账,refundId=" + refundId + "," + result);
							} catch (Exception syncEx) {
								logger.error("处理退款写账出错，refundId=" + refundId, syncEx);
							}
						}
					}
				} catch (Exception e) {
					logger.error("处理退款写账出错。", e);
					isHasError = true;
					e.printStackTrace();
				}
			}
