...
@Service
public class EntMsgSenderJob implements BaseJob {
	private static Logger logger = LogManager.getLogger(EntMsgSenderJob.class);
	private static ReentrantReadWriteLock rtw = new ReentrantReadWriteLock();
	private static List<String> MSG_ERROR_CODES = Arrays.asList(SystemConfig.getPara("EntMsgErrorCode").split(","));
	...
	private boolean sendMsgAt(int startHour, int startMin, int endHour, int endMin) {
		if ((startHour == 0 && startMin == 0 && endHour == 0 && endMin == 0) || (DateUtils.addMinutes(DateUtils.addHours(DateUtils.startOfDate(new Date()), startHour), startMin).getTime() <= DateUtils.addHours(new Date(), 0).getTime() && DateUtils.addMinutes(DateUtils.addHours(DateUtils.startOfDate(new Date()), endHour), endMin).getTime() >= DateUtils.addHours(new Date(), 0).getTime())) {
			return true;
		} else {
	...
	public void execute(JobEntity jobEntity) {
		...
		try {
			...
			List<EntMsg> list = entMsgService.getByPage(filter);
			msgCount = list.size();
			if (list != null && list.size() >= 0) {
				Iterator<EntMsg> it = list.iterator();
				while (it.hasNext()) {
					try {
						EntMsg entMsg = it.next();
						String content = entMsg.getContent();
						String touser = entMsg.getUserId();
						if (StringUtils.isEmpty(touser) || StringUtils.isEmpty(content)) {
							entMsg.setRemark("参数不能为空");
							entMsg.setUpdateTime(new Date());
							entMsgService.update(entMsg);
							continue;
						}

						JSONObject jsonobj = EntMsgSender.sendMsg(entMsg);
						entMsg.setResendTimes(entMsg.getResendTimes() + 1);
						entMsg.setSendTime(new Date());
						entMsg.setUpdateTime(new Date());
						if (jsonobj != null) {
							int sendResult = jsonobj.getInt("errcode");
							entMsg.setSendResult(String.valueOf(sendResult));
							entMsg.setSendTime(new Date());
							if (0 == sendResult) {
								entMsg.setStatusId(1);
								entMsg.setSendResult(String.valueOf(sendResult));
								entMsg.setRemark(jsonobj.getString("errmsg"));
								successSendCount++;
							}
							// 发送失败,只有在错误列表中的编码的消息才会重发
								...
								//其它错误一律不重发消息
								entMsg.setStatusId(-2);
								entMsg.setSendResult(String.valueOf(sendResult));
								entMsg.setRemark(jsonobj.getString("errmsg"));
							}
						} else {
							entMsg.setSendResult("-1");
							entMsg.setRemark("发送失败，未接收到响应信息。");							
						}
						entMsgService.update(entMsg);
					} catch (Exception e) {
						logger.error(e);
